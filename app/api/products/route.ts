...
function toBrand(value: string | undefined): Brand | undefined {
  // Preserve any non-empty brand value coming from Notion.
  const v = (value ?? '').trim()
...
function toCategory(value: string | undefined): Category {
  ...
  const normalized =
    v === 'games' ? 'game'
      : v === 'game' ? 'game'
        : v === 'essentials' ? 'essential'
          : v === 'essential' ? 'essential'
            : v === 'sw' ? 'software'
              : v === 'watch list' ? 'watchlist'
                : v === 'watch-list' ? 'watchlist'
              : v
  ...
  return (allowed.includes(normalized as Category) ? (normalized as Category) : 'essential')
}
...
export async function GET() {
  try {
    ...
    // Fetch ALL pages (Notion paginates database queries), but don't allow the
    // request to hang forever if Notion is slow.
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error(`Notion query timed out after ${NOTION_TIMEOUT_MS}ms`)),
          NOTION_TIMEOUT_MS,
        ),
      ),
    ])
    ...
    const products: Product[] = allResults.map((page: any) => {
      ...
      return {
        ...
        url,
        coverImage,
        darkModeCoverImage: darkModeCoverImage || undefined,
        date,
